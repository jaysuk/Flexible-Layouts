...
- **Widgets** — built-in DWC panels, pages/tabs from other plugins, custom-panel groups, command
  buttons (G-code / macros / HTTP / open-URL), a Pronterface-style **jog** control, input fields
  (run a command with a value, or set a `global` variable), live value read-outs, live charts (with
  CSV download), text / images, and web embeds.
