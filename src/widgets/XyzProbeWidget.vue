<template>
	<div class="xpw-root fill-height d-flex flex-column px-2 py-1" :class="{ 'xpw-frozen': disabledNow }">
		...
		<v-expansion-panels variant="accordion" class="xpw-settings flex-grow-1">
			<v-expansion-panel :title="$t('plugins.flexibleLayouts.xyzProbe.settingsTitle')">
				<v-expansion-panel-text>
					<v-row dense>
						<v-col cols="6"><v-text-field v-model.number="feedrate" type="number" :min="1" density="compact" variant="outlined" hide-details :label="$t('plugins.flexibleLayouts.xyzProbe.feedrate')" :disabled="disabledNow" /></v-col>
						<v-col cols="6"><v-text-field v-model.number="searchMargin" type="number" :min="0.1" :step="0.5" density="compact" variant="outlined" hide-details :label="$t('plugins.flexibleLayouts.xyzProbe.searchMargin')" :disabled="disabledNow" /></v-col>
					</v-row>
					<v-row dense class="mt-1">
						<v-col cols="4"><v-text-field v-model.number="plateWidth" type="number" :min="1" density="compact" variant="outlined" hide-details :label="$t('plugins.flexibleLayouts.xyzProbe.plateWidth')" :disabled="disabledNow" /></v-col>
						<v-col cols="4"><v-text-field v-model.number="plateHeight" type="number" :min="1" density="compact" variant="outlined" hide-details :label="$t('plugins.flexibleLayouts.xyzProbe.plateHeight')" :disabled="disabledNow" /></v-col>
						<v-col cols="4"><v-text-field v-model.number="plateThickness" type="number" :min="0.01" :step="0.01" density="compact" variant="outlined" hide-details :label="$t('plugins.flexibleLayouts.xyzProbe.plateThickness')" :disabled="disabledNow" /></v-col>
					</v-row>
					<v-row dense class="mt-1">
						<v-col cols="6"><v-text-field v-model.number="xOffset" type="number" :step="0.01" density="compact" variant="outlined" hide-details :label="$t('plugins.flexibleLayouts.xyzProbe.xOffset')" :disabled="disabledNow" /></v-col>
						<v-col cols="6"><v-text-field v-model.number="yOffset" type="number" :step="0.01" density="compact" variant="outlined" hide-details :label="$t('plugins.flexibleLayouts.xyzProbe.yOffset')" :disabled="disabledNow" /></v-col>
					</v-row>
...
import {
	buildXyzProbeCommand, deployXyzProbeMacros, xyzProbeMacrosMissing, XYZ_PROBE_CORNER_FILE,
...
.xpw-settings { min-height: 0; overflow-y: auto; }
